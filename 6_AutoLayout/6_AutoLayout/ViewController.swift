...
//  6_AutoLayout
//
//  Created by Janis Cimbulis on 11/10/2020.
//
...
class ViewController: UIViewController {
    ...
    func createLabels() {
        ...
        for (index, label) in labelArray.enumerated() {
            ...
            if index > 0 {
                // 2nd label and onwards
                label.topAnchor
                        .constraint(equalTo: labelArray[index - 1].bottomAnchor, constant: 10)
                        .isActive = true
            }
            else if index == labelArray.count {
                // last label
                label.bottomAnchor
